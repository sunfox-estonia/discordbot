				...
				checkProfileExists(fetchedUser.user.id, function (error) {
					if (error) {
						const locales = {
							...
							et: 'Kasutaja profiili kontrollimisel on tekkinud viga.',
						};
						...
						createProfile(fetchedUser.user.id, function (error) {
							if (error) {
								const locales = {
									...
									et: 'Kasutaja uue profiili loomisel on tekkinud viga.',
								};
								interaction.reply({ content: locales[interaction.locale] ?? error, ephemeral: true });
							} else {
								interaction.reply({ content: 'Создан новый профиль пользователя для ' + fetchedUser.user.nickname ?? fetchedUser.user.username, ephemeral: true });
