                    ...
                    if (error) {
                        ...
                        addRegistrationDecline(user_uid, event_id ,function(error){
                            if (error) {
                                ...
                                interaction.reply({ content: `Вы отказались от участия в мероприятии. Вы всегда можете изменить свое решение!`, ephemeral: true });
                            }
                        });
                    }
                });
...
checkEventDeclinedProfileExists = function (user_uid, callback) {
	...
	database.query(sql2, [user_uid], (error2, results, fields) => {
		if (error2) {
			...
			callback("Данный профиль не существует. Невозможно взаимодействовать с регистрацией на мероприятие.");
			return;
		} else {
			callback(null);
			return;
...
addRegistrationDecline = function(user_uid, event_id, callback) {
	// Add achivement for user
	let sql3 = "INSERT INTO events_usr (user_uid, event_id, user_status) VALUES (?,?,'0');";
    database.query(sql3, [user_uid,event_id], (error3, pingback) => {
        if (error3) {
            callback("Ошибка взаимодействия с регистрацией на мероприятие.");
            return;
        } else {
			callback(null);
		}
